...
- Run state is server-authoritative and persists in one SQLite 3 database on the mounted Railway volume so restarts and deploys do not destroy runs. Phase 01 pins `rusqlite`, the bundled SQLite source/version/checksum, compile options, and license evidence; the game server never depends on an unpinned system SQLite. The database, `-wal`, and `-shm` files live in one private service-owned directory and are never placed on an unqualified network filesystem.
- Phase 01 must qualify SQLite on the actual staging volume before gameplay depends on it: verify advisory locking, `fsync`/durable-commit behavior, crash recovery, deployment replacement, checkpoint behavior, and the representative plus maximum-state write workload. Failure of that gate requires an explicit contract decision for managed PostgreSQL or another mature transactional store; implementing a custom WAL is not the fallback.
- One supervised blocking persistence thread owns the sole writable SQLite connection. Tokio tasks never call SQLite directly and never use one `spawn_blocking` job per mutation. A bounded `128`-request channel allows at most one in-flight commit per active session, with `64` ordinary slots and `64` slots reserved for disconnect, token handoff, expiry, leave, end, shutdown, and recovery work. Ordinary mutation returns `persistence_unavailable` before staging when its partition is full.
- A new database fixes `page_size=4096` and `auto_vacuum=INCREMENTAL` before schema creation. Every open enforces `journal_mode=WAL`, `synchronous=FULL`, `foreign_keys=ON`, `trusted_schema=OFF`, `busy_timeout=5000`, `wal_autocheckpoint=8192`, and `journal_size_limit=67108864`; startup verifies the effective values instead of assuming each pragma succeeded. There are no long-lived read transactions, and all SQL values are bound parameters.
- `PersistedSessionSnapshot` is a server-only, explicitly versioned representation capped at `256 KiB` before database submission. It includes authoritative gameplay state, logical tick, state revision, `next_event_id`, each player's committed `next_input_seq`, optional active-run RNG algorithm/version/state, pack identity, per-seat identity/token/connection/grace state, absolute `created_at_unix_ms`/`expires_at_unix_ms`/`last_observed_unix_ms`, and remaining monotonic gameplay-deadline durations. Uncommitted unresolved-ledger entries are never persisted; after crash/restore clients retry from the committed frontier. Lobby snapshots contain no run RNG. It is distinct from `ClientResyncState`.
- The initial database schema stores one current post-transition snapshot row per session with `session_id`, `save_version`, `state_revision`, `status`, bounded snapshot bytes, SHA-256 of those exact bytes, creation/update/expiry timestamps, and retention metadata. `status` is `active`, `ended`, or `quarantined`. The complete seat state is inside the same snapshot so gameplay, replay frontiers, RNG position, token handoff, and lifecycle state change atomically. Ended rows act as tombstones through retention. Schema migrations use positive `database_schema_version` stored in SQLite `PRAGMA user_version`, start at `1`, are distinct from `save_version`, and run transactionally before restoration. Future/zero versions fail closed except that a retained rollback build may carry an explicitly tested open/read path for exactly the next additive schema under the rollback rule; migrations never downgrade or destructively rewrite a newer database.
- Persist only `HMAC-SHA-256` digests of rejoin tokens. Each occupied seat records its Steam-identity/session/player binding, non-secret `client_join_nonce`, current token key ID and generation, optional pending-handoff token key ID and generation, absolute token expiry, connection generation/state, `last_connection_observed_unix_ms`, and optional absolute `rejoin_grace_expires_at_unix_ms`. Compare digests in constant time. Raw bearer tokens exist only on the client and transiently on the server while generating, serializing, validating, or echoing a token; they are never logged or persisted. The HMAC key is a managed server secret outside SQLite; retain the current and previous key until all snapshots/tokens using the previous key expire.
- Clients never submit saved run state; resume always happens through the rejoin flow into a server-restored session. Longer-term resume after rejoin tokens expire is post-MVP; if added, re-admittance is by validated Steam identity, never by extending token lifetime.
...
- Each state-changing operation is a transaction: validate against committed in-memory state; clone or journal only the bounded fields it can change; stage gameplay state, RNG position, consumed `input_seq`, revision, snapshot bytes/checksum, and outputs; reserve every recipient's output capacity; then submit one staged post-state to persistence. The persistence thread batches ready submissions for at most `20 ms`, writes at most one row per session in one SQLite transaction, and reports success only after `COMMIT` returns under `synchronous=FULL`. Session owners then atomically install their staged in-memory states and enqueue outputs. No observer may read staged state and no externally visible `state`, `event`, `input_result`, token handoff, or leave acknowledgement is emitted before commit.
- A failed SQLite begin/write/commit rolls back the complete persistence group. Every affected session discards its staged gameplay/RNG mutation, consumes no `input_seq`, increments no revision, releases output reservations, clears the unresolved suffix admitted for that failed group, resets each admission frontier to its lowest discarded sequence, and returns correlated recoverable `persistence_unavailable` with that retryable `next_input_seq`. All higher discarded inputs are retried from that frontier; none remains ambiguously admitted.
- Persistence retries are bounded to three attempts with delays of `100 ms`, `500 ms`, and `2 s`. While retrying, an affected session accepts heartbeats/resync but rejects new state-changing work with correlated `persistence_unavailable`. After the third failure it enters `PersistenceBlocked` for at most `30s`; one successful probe transaction restores service. Otherwise the owner commits no further mutation, sends `session_terminated` when the writer is available, closes with `1011`, and leaves the last committed SQLite row untouched for operator recovery.
- Accounted SQLite artifacts—the database, `-wal`, `-shm`, bounded online-backup temporary output, and diagnostics—are capped at `1 GiB`. Ordinary creation/mutation stops before accounted use exceeds `768 MiB` or provider free space falls below `256 MiB`, preserving headroom for the maximum `64` bounded terminal rows, rollback, checkpoint, and cleanup. Session creation reserves `1 MiB` in this accounting before admitting its first seat. Checkpoint work is incremental and bounded; it never runs `VACUUM` on the request path. Cleanup processes at most `256` ended rows per pass, and no active or quarantined row is evicted implicitly.
- Startup acquires the exclusive process lock, opens SQLite, lets SQLite recover its WAL, verifies required pragmas, runs bounded `quick_check`, and validates at most the `64` active rows for size, SHA-256, save version, timestamps, and domain invariants before admission. A structurally healthy database with an invalid session row preserves that row by marking it `quarantined`, excludes it from capacity, emits one redacted operator error, and makes join/rejoin return `session_not_found`. At most `64` rows and `256 MiB` may be quarantined. Database-level corruption, inability to durably mark a bad row, or exceeded quarantine/storage bounds leaves bytes untouched and keeps `/readyz` unsuccessful for operator restore; the server never deletes evidence to become ready.
- Clean shutdown runs SQLite `wal_checkpoint(TRUNCATE)` after all required commits, closes the connection, and only then releases the process lock. A crash relies on SQLite WAL recovery; it never treats an uncommitted transaction as acknowledged. Provider backups are defense against volume loss/corruption and are never used to resurrect sessions deleted beyond retention.
...
- Single-instance enforcement: acquire one exclusive OS file lock in the SQLite directory before opening the database, restoration, or admission, and hold it until the persistence thread has checkpointed/closed SQLite during clean shutdown. SQLite locking is defense in depth, not a replacement for the process lock. Multiple replicas, network filesystems, or a volume whose locks are not mutually visible are unsupported; Phase 01 must prove that an overlapping staging deployment cannot obtain the lock or write the database. `/readyz` remains unsuccessful until the lock is held and restoration is complete.
...
- Operations baseline: expose `/healthz` for process liveness and `/readyz` for service readiness. Readiness requires the exclusive process lock, completed SQLite recovery/row validation, writable persistence, and ability to serve existing sessions; exhausted session/player admission capacity does not make the service unready. New creation/join receives `server_full`, while reserved-seat rejoin remains available. Use structured logs and monitor deploy status, missed ticks, persistence commit/checkpoint latency, queue saturation, error rate, disconnect rate, reconnect failures, and admission capacity separately.
- Graceful shutdown: mark unready, stop new creation/join, send the `server_restarting` notice with `retry_after_ms`, commit pending required transitions, await owned tasks, perform the bounded final SQLite checkpoint/close, release the process lock, and exit within `20s`. Connections close with WebSocket `1012` only after their notice is handed to the reserved writer-control slot. If the deadline is reached, exit non-zero without claiming a clean checkpoint; SQLite recovery handles any committed WAL on the successor.
...
- The committed SQLite session snapshot preserves current/pending generations and bindings, so a crash cannot silently promote, lose, or extend a handoff. Explicit leave, session end, and absolute expiry invalidate both generations.
...
| --- | --- |
| `ClientView` | `phase`, `self: PlayerView`, `party: PlayerView[1..=4]`, `available_actions: ActionKind[0..=16]`, optional recipient-only `self_inventory: InventorySlotView[0..=4]`, and exactly one phase detail; `party` contains every occupied seat including `self`, sorted by lexical `player_id`, with unique IDs |
...
- On each simulation tick, the owner forms one session-wide batch containing at most one selected gameplay action per connected player plus the superseded prefix entries required above. It stages players in lexical `player_id` order against the preceding staged result and encodes bounded outputs before commit. It must reserve the exact required writer slots/bytes for every recipient before submitting the staged post-state to SQLite; failure disconnects only the slow recipient and retries staging without consuming any sequence. It then commits all gameplay/RNG changes, consumed sequences, one revision, and the complete post-state in the grouped SQLite transaction. Any persistence failure releases reservations and rolls back the whole batch. Semantic rejection of one input does not roll back another valid input; the event-bound failure rule above is the sole whole-batch semantic rollback.
...
- A server delayed by more than one tick performs one immediate catch-up tick, records all additionally dropped logical ticks in `server_tick_time_dropped_total`, emits one bounded `time_dropped` diagnostic event, and resumes the next scheduled tick. Dropped ticks do not advance logical time or movement, but server-monotonic deadlines still elapse. The owner converts elapsed deadlines into explicit `DeadlineExpired { kind, deadline_id }` state-machine inputs; `game_core` never reads a clock. The authoritative deadline ordering point is when the session owner dequeues an already parsed, authenticated, and rate-admitted command from its bounded mailbox. The owner samples its injected monotonic clock at that point and then performs sequence/admission validation against committed state: a valid expected input observed strictly before the deadline stages before movement/interactions and expiry; equality or later resolves expiry first. Frame-arrival timestamps never backdate an input, and validation or mailbox waiting time is part of the client's latency budget.
...
- TMX/XML parsing disables DTDs, external entities, XInclude, and all external resource resolution. Untrusted image/audio validation and decode use the worker sandbox below with a `2s` per-file wall timeout, process memory cap `256 MiB`, concurrency `2`, and declared decoded-output caps even when headers lie; timeout or limit kills and reaps the complete worker process tree before its slot is reused.
- Every validation worker starts with an empty allowlisted environment, fixed locale/timezone, no secrets, a fresh private temporary directory, and only the explicitly allowlisted pre-opened input/output/IPC handles. The supervisor closes all other descriptors/handles, reserves worker RSS, decoded output, IPC, and temporary bytes before spawn, and applies a shared `1 GiB` worker-reservation ceiling plus `256 MiB` temporary-disk ceiling. Unreserved jobs remain in a bounded queue of `8` packs/`512 MiB` compressed bytes before rejection.
- On supported Linux, the worker is initialized before hostile bytes are read, sets `PR_SET_NO_NEW_PRIVS`, applies `RLIMIT_AS`, `RLIMIT_CPU`, `RLIMIT_FSIZE`, and `RLIMIT_NOFILE`, confines filesystem access with a Landlock ruleset to only the private temporary directory and pre-opened handles, then installs a seccomp-BPF policy that denies network/socket creation, `execve`/`execveat`, tracing, namespace/mount operations, and process creation while permitting only explicitly required thread creation. The supervisor uses `pidfd`/process-group termination where available and treats inability to install every required control as `validation_resource_limit`.
- On supported Windows, the supervisor creates the worker suspended with an explicit inherited-handle list, an AppContainer/restricted low-integrity token with no network or broad filesystem capabilities, and process-mitigation policies; assigns it to a Job Object with kill-on-close, active-process limit `1`, memory/time limits, and no breakaway; then resumes it. Failure to apply the token, handle list, mitigations, or Job Object is `validation_resource_limit`.
- Release builds support sandboxed validation only on the locked Ubuntu and Windows release baselines. An unsupported kernel/OS or unavailable sandbox primitive fails closed before parsing and explains that pack validation is unavailable; no command-line flag, environment variable, developer setting, or release feature may weaken this policy. Synthetic sandbox-bypass fixtures are permitted only in non-release test binaries proven absent from packages. Validation and runtime loading enforce the same limits; community packs remain hostile after hub validation.
...
Schema validation occurs in this order: archive/path/resource bounds, JSON syntax/duplicate keys, structural field/type/bounds, identifier uniqueness, cross-reference resolution, TMX semantics, graph/automatic-transition and worst-case event/projection-size validation, license/attestation policy, then canonical checksum. ZIP/path/checksum and JSON/TMX/XML parsing/semantic validation run only in the fail-closed worker sandbox described above, with at most `4` aggregate workers and `2` media workers subject to the shared reservation ceiling. Each stage has a `5s` wall deadline and one pack has a `15s` aggregate deadline; an individual aggregate worker has a `512 MiB` RSS cap. Deadline, cancellation, worker crash, or limit kills and reaps the subprocess tree, removes temporary output, returns `validation_resource_limit`, and installs nothing; parser work never runs in a non-killable in-process thread. A failure returns one bounded path-based diagnostic and performs no media decode or network access before cheaper structural checks pass. For a community Tier 1 import, classification permits only the root manifest plus its listed story/character documents and rejects `theme:` includes, `themes/*.json`, and every map/tileset/media/other member; Tier 2 classifies permission from the complete validated archive member set, so an unreferenced custom or theme file cannot bypass attestation.
...
- Rust toolchain: `1.97.1`, edition `2024`, and MSRV `1.97.1`, pinned in `rust-toolchain.toml`. The virtual workspace explicitly sets `resolver = "3"`; it never relies on member editions to infer a resolver. Toolchain changes are explicit contract updates.
...
- Before Phase 01 completes, build documentation pins `rusqlite` and its bundled SQLite engine source/version/checksum, compile options, update procedure, and license evidence. Server CI prints and verifies the effective SQLite version/compile options and runs the persistence qualification with the same build; production never falls back to a host SQLite library.
...
- Phase 01 completion evidence also proves the supported Linux/Windows sandbox profiles fail closed, qualifies SQLite on the actual staging volume, records viable access to the `256` authorized Steam test accounts and `19` real limiter-keyed source IPs required by the benchmark, and identifies the protected Windows signing service. These are entry gates, not work deferred to Phases 13-18.
...
- Every final candidate produces `artifacts/releases/<version>/rollback-manifest.json` before promotion. It records the candidate and retained-prior Git SHA, server image digest, Linux/Windows package and Steam depot manifest IDs, protocol/content/rules/save/settings versions, SQLite `database_schema_version`, built-in pack checksums, non-secret configuration snapshot digest, database/volume migration state, exact rollback commands, operator, and evidence paths. The prior server image and both prior depots remain immutable and available until the next release clears its rollback window.
- The server rollback procedure marks the candidate unready, stops admission, commits pending required transitions, performs the bounded SQLite checkpoint/close, releases the process lock, deploys the exact retained digest/config, opens and restores every session through the retained build's compatible database/save readers, verifies `/healthz` and `/readyz`, and runs create/join/rejoin plus one continued-run transcript before traffic returns. Clean process-start-to-ready must meet the `20s` clean-restore gate; the complete staging rollback drill, including smoke checks, must finish within `5 minutes`.
...
- A candidate is rollback-compatible only when the retained server can open the candidate's SQLite `database_schema_version`, read every authoritative save version the candidate may write, and use a compatible protocol/rules/pack tuple. Database schema changes are additive/expand-and-contract throughout the rollback window: before a candidate migrates to schema `N`, the deployed retained build must already pass its prepared exactly-next-schema `N` open/read path while continuing to create schema `N-1`. A save-version change likewise requires that, before any release writes save version `N`, the retained rollback build already contains and passes a read-only `N` migrator while still writing `N-1`. If that preparatory release was not deployed, the version-changing candidate cannot pass Release-ready. An older client may preserve and ignore a newer settings file and use canonical defaults as already specified, because settings are non-authoritative. No rollback path may rewrite a newer settings/save file destructively or restore an older snapshot that loses an acknowledged transition.
...
| Durable active-session SQLite rows/database WAL | Railway volume; game service and authorized operator | Delete ended rows after committed session end plus `24 hours`; encrypted provider backups expire within `30 days` and are not used to resurrect deleted sessions |
...
This section is a bootstrap contract only while `les-perissables-hub` does not exist. Phase 19 moves its internal application, database, OAuth, moderation, QA, security, and operations requirements into that repository and removes the duplicated internals here; this game contract retains only game-facing pack, attestation, publication, and provisioning interfaces plus the public-launch dependency.
...
- Tier 1 (reuse-only) at first creator release: packs may add new stories, new character stat/spell combinations, and flavor, but must reference already-shipped maps, themes, sprites, audio, and spells. A community Tier 1 archive may contain only its manifest plus listed story/character documents; `theme:` includes and `themes/*.json`, map, tileset, image, audio, or other custom members are forbidden. No new asset files or composed theme manifests are allowed. Built-in release packs are trusted project artifacts and are not classified by this community-import rule.
...
- The current save version and the immediately previous positive version when one exists restore exact authoritative state after injected SQLite begin/write/commit/checkpoint failures; corrupt/future/oversized rows and database-level corruption fail safely.
